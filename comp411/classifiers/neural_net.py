...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        sum_of_scores = np.sum(np.exp(scores), axis=1, keepdims=True)
        softmax = np.exp(scores) / sum_of_scores
        data_loss = np.mean(-np.log(softmax[np.arange(N), y]))
        #loss = data loss + regularization_loss
        #regularization_loss = regularization_strength * L2_regularization
        #L2 reg --> sigma(sigma||W_(k,l)||^2) 
        L2_regularization = np.sum(W1**2) + np.sum(W2**2) + np.sum(W3**2)
        loss = data_loss + (reg * L2_regularization)
        ...
        grad_scores = softmax.copy()
        grad_scores[np.arange(N), y] -= 1
        grad_scores /= N
        
        grads["W3"] = np.dot(h2.T, grad_scores) + (reg * W3 * 2)
        grads["b3"] = np.sum(grad_scores, axis=0)
        
        grad_h2 = np.dot(grad_scores, W3.T)
        grad_relu2 = (h2 > 0) * grad_h2

        grads["W2"] = np.dot(h1.T, grad_relu2) + (reg * W2 * 2)
        grads["b2"] = np.sum(grad_relu2, axis=0)
        
        grad_h1 = np.dot(grad_relu2, W2.T)
        grad_relu = (h1 > 0) * grad_h1
        
        grads["W1"] = np.dot(X.T, grad_relu) + (reg * W1 * 2)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            r = np.random.choice(num_train, batch_size, True)
            X_batch = X[r]
            y_batch = y[r]
